...
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "Planned Vision MP4 \(local CUDA\)" "montage example local CUDA render path"
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "Planned Vision MP4 \(local CPU\)" "montage example local CPU render path"
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "Automatic source sampling" "montage example automatic sampling selector"
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "scene threshold" "montage example scene detection controls"
...
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "summarizeRenderFailure" "montage example concrete workflow failure summary"
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") "mismatch between text model" "montage example mmproj mismatch visibility"
Assert-FileContains (Join-Path $montageExampleRoot "src\ofApp.cpp") '2>&1' "montage example worker output capture"
...
Assert-FileContains (Join-Path $scriptRoot "run-video-montage-workflow.ps1") "Get-SceneSamplePlan" "montage workflow scene-aware sampling"
Assert-FileContains (Join-Path $scriptRoot "run-video-montage-workflow.ps1") "SamplingFallbackReason" "montage workflow visible sampling fallback"
