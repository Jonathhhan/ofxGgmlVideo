...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlVideo-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlVideo tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlVideo tests" $command
} else {
	Write-Step "Configuring ofxGgmlVideo tests"
	Invoke-CheckedNative "cmake configure ofxGgmlVideo tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlVideo tests"
	Invoke-CheckedNative "cmake build ofxGgmlVideo tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlVideo tests"
	Invoke-CheckedNative "ctest ofxGgmlVideo tests" {
