...
function Test-RuntimeSmokeReady {
	return (Test-Path -LiteralPath (Join-Path $addonRoot "src\ofxGgmlVideo\ofxGgmlVideoTypes.h") -PathType Leaf) -and
		(Test-Path -LiteralPath (Join-Path $addonRoot "src\ofxGgmlVideo\ofxGgmlVideoUtils.cpp") -PathType Leaf) -and
...
function New-DryRunSummary {
	...
	return [ordered]@{
		Name = "ofxGgmlVideo runtime smoke"
...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlVideo-runtime-smoke"
}
...
if ($DryRun) {
	...
	Write-Step "ofxGgmlVideo runtime smoke plan"
...
$results += Invoke-SmokeStep -Name "video doctor" -Arguments $doctorArgs
...
if ($Json) {
	...
	Write-Step "ofxGgmlVideo runtime smoke summary"
	Write-Host "  Backend: $($summary.Backend)"
	Write-Host "  ModelBacked: $($summary.ModelBacked)"
