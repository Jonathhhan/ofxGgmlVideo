...
$script = Join-Path $scriptRoot "run-video-runtime-smoke.ps1"

Write-Step "Video runtime smoke dry-run"
$textOutput = & $script -DryRun 2>&1 6>&1 | Out-String
Assert-Contains $textOutput "ofxGgmlVideo runtime smoke plan" "runtime smoke dry-run"
...
if ($summary.ModelBacked) {
	throw "Video runtime smoke should not claim model-backed inference yet."
}
if (!($summary.NextCommands -contains "scripts\run-video-runtime-smoke.bat -Json -SummaryOnly")) {
	throw "JSON dry-run did not include the runtime smoke command."
}

Write-Step "Video runtime smoke contract passed"
